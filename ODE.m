...
if iex == 5
    ...
    for ii=1:length(xvec)-1
        ...
        yvec(ii+1)=yvec(ii)+deltax*dydx(xvec(ii),yvec(ii));
    ...
    % Initailize x
    deltax = 0.09; % smaller discretization (more accurate results)
    xvec = [x0:deltax:x(end)];
    yvec(1) = y0;
    % Inline functions for k1,k2,k3,k4
    f = dydx;
    k1 = @(x,y) deltax*f(x, y);
    k2 = @(x,y) deltax*f(x+0.5*deltax, y+0.5*k1(x,y));
    k3 = @(x,y) deltax*f(x+0.5*deltax, y+0.5*k2(x,y));
